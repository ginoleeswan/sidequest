...
/**
 * Both glyphs written out, not one plus a suffix.
 *
 * `StatusActions` used to build the unselected name as
 * `` `${icon}-outline` ``, and the icon subset is generated by scanning
 * this tree for quoted glyph names — so a name assembled at runtime is
 * invisible to it. Two of the three survived by accident, because
 * `bookmark-outline` and `game-controller-outline` are written
 * literally somewhere else; `checkmark-circle-outline` was not, so it
 * never shipped in the font and Finished sat there with no icon while
 * its two neighbours had one.
 *
 * Written out, both states are literals the generator can see, and the
 * next status added cannot repeat this quietly.
 */
export const STATUS_META: Record<
  LibraryStatus,
  { label: string; icon: string; iconOutline: string }
> = {
  wishlist: {
    label: 'Want to play',
    icon: 'bookmark',
